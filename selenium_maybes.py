...
"""
NOTE
NOTE the purpose of this is to iterate over the output of "selenium_multithread.py"
 and see if there were any mistakes while finding out the "Maybe" switches to minimalize
 as much manual labor needed. 

NOTE that sometimes "selenium_multithread" may be inaccurate because Solarwinds 
 can fail to load in time. 
...
data_file_name = "result_agg_v3.csv"
...
with open(data_file_name, newline='') as f:
    reader = csv.reader(f)
    for row in reader:
        catalyst_switches.append(row)
total_switches = len(catalyst_switches) - 2
print(f"{total_switches} switches!")
...
def selenium_function(catalyst_switches, start, level):
    ...
    for i in range(start, total_switches, level): #iterate through each ip_address. 
        ...
        if catalyst_switch[2] != "Maybe": continue
        ...
        if start == 1:
            ...
            print(f"Progress: {count * level} devices, time elapsed: {round(time_elapsed/60)} min.")
        ...
        if (count % 24 == 0): #recommended to leave this below about 46. 
            ...
            time.sleep(1)
            driver.get('https://usaslcnet3.goldbar.barrick.com/ui/search?q=10.12.90.4')
            time.sleep(26) #wait for the page to load properly before searching. 
        ...
        device_name_page_count += driver.page_source.count(device_name + ' ') #trunks count, assuming the naming convention is correct
...
catalyst_switches[-1][2] = f"{in_solarwinds/(total_switches-1)}"
